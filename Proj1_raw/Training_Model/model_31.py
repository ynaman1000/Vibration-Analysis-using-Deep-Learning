...
doBatchNorm = [False]       # for performing batch normalization on each layer
if doBatchNorm:
    trackStats = False     # for tacking mean and variance of hidden units over mini-batches
...
padFront = [True]        # for padding layers without activation function in front with same number of units as in the end

numHypParamSamples = 1
if numHypParamSamples == 1:
    initLR = 2e-3           # initial learning rates
    lrUpdateCoeff = 5        # number by which learning rate will be divided on every updation
    lrUpdatePeriod = 5000       # number of iterations after which learning rate will be updated
...
class Model(torch.nn.Module):
    ...
    def __init__(self, inputSize, outputSize, hlwaf, n1, hlwoaf, n2, bn, tStats, activation_fn, pad_f):
        super(Model, self).__init__()
        if bn:
            # Defining hidden layers without activation function
            if pad_f and hlwoaf > 0:
                self.layers = torch.nn.Sequential(torch.nn.BatchNorm1d(inputSize, track_running_stats=tStats), torch.nn.Linear(inputSize, outputSize*(n2**1)), torch.nn.BatchNorm1d(outputSize*(n2**1), track_running_stats=tStats))
                for i in range(2, hlwoaf+1):
                    self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**(i-1)), outputSize*(n2**i)), torch.nn.BatchNorm1d(outputSize*(n2**i), track_running_stats=tStats))
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**hlwoaf), n1), torch.nn.BatchNorm1d(n1, track_running_stats=tStats), activation_fn)
            else:
                self.layers = torch.nn.Sequential(torch.nn.BatchNorm1d(inputSize), torch.nn.Linear(inputSize, n1), torch.nn.BatchNorm1d(n1, track_running_stats=tStats), activation_fn)
            # Defining hidden layers with activation function
            for _i in range(hlwaf-1):
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n1, n1), torch.nn.BatchNorm1d(n1, track_running_stats=tStats), activation_fn)
            # Defining hidden layers without activation function
            if hlwoaf > 0:
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n1, outputSize*(n2**hlwoaf)), torch.nn.BatchNorm1d(outputSize*(n2**hlwoaf), track_running_stats=tStats))
                for i in range(hlwoaf-1):
                    self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**(hlwoaf-i)), outputSize*(n2**(hlwoaf-i-1))), torch.nn.BatchNorm1d(outputSize*(n2**(hlwoaf-i-1)), track_running_stats=tStats))
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**1), outputSize))
            else:
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n1, outputSize))
        else:
            # Defining hidden layers without activation function
            if pad_f and hlwoaf > 0:
                self.layers = torch.nn.Sequential(torch.nn.Linear(inputSize, outputSize*(n2**1)))
                for i in range(2, hlwoaf+1):
                    self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**(i-1)), outputSize*(n2**i)))
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**hlwoaf), n1), activation_fn)
            else:
                self.layers = torch.nn.Sequential(torch.nn.Linear(inputSize, n1), activation_fn)
            # Defining hidden layers with activation function
            for _i in range(hlwaf-1):
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n1, n1), activation_fn)
            # Defining hidden layers without activation function
            if hlwoaf > 0:
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n1, outputSize*(n2**hlwoaf)))
                for i in range(hlwoaf-1):
                    self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**(hlwoaf-i)), outputSize*(n2**(hlwoaf-i-1))))
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(outputSize*(n2**1), outputSize))
            else:
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n1, outputSize))
...
for bN in doBatchNorm:
    for aF in actFuns:
        for pF in padFront:
            ...
            if do_analysis:
                file_name = "Analysis_files/new/_bN-" + repr(bN) + "_actFun-" + repr(aF) + "_pF-" + repr(pF) + "_case-" + repr(case) \
                ...
                analysis_file.write(','.join(["Batch Normalization", repr(bN), "\n"]))
                analysis_file.write(','.join(["Activation Function", repr(aF), "\n"]))
                analysis_file.write(','.join(["Pad layers in front", repr(pF), "\n", "\n"]))
            ...
            for _i in range(numHypParamSamples):
                ...
                if numHypParamSamples > 1:
                    ...
                    L1 = np.random.randint(1, 6)                   # number of hidden layers with activation function
                ...
                model = Model(dimInput, dimOutput, L1, N1, L2, N2, bN, trackStats, aF, pF)
                model.to(device)
                my_optimizer = torch.optim.Adam(model.parameters(), lr=initLR)
                ...
                for t in range(numIter):
                    ...
                    if t%lrUpdatePeriod == lrUpdatePeriod-1 and lrUpdateCoeff > 1:
                        adjust_learning_rate(my_optimizer, lrUpdateCoeff)
