...
num_iter = 40000
init_lrs = [1e-2]       # = initial learning rates * lr_update_coeff
# num_hl_with_act = [2, 3, 4]   # number of hidden layers with activation function
...
num_units_with_act = [170]                  # number of units per hidden layer with activation function
num_hl_without_act = [0, 1, 2, 3]   # number of hidden layers without activation function
# num_hl_without_act = [2]          # number of hidden layers without activation function
...
act_fns = [torch.nn.ELU(), torch.nn.SELU(), torch.nn.ReLU(), torch.nn.PReLU(), torch.nn.Softplus()]     # activation functions
# act_fns = [torch.nn.ELU(), torch.nn.SELU(), torch.nn.ReLU()]                                          # activation functions
lr_update_coeffs = [5]          # number by which learning rate will be divided on every updation
lr_update_periods = [5000]      # number of iterations after which learning rate will be updated
...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, hlwa, n, hlwoa, activation_fn):
        super(Model, self).__init__()
        self.layers = torch.nn.Sequential(torch.nn.Linear(input_size, n), activation_fn)
        for i in range(hlwa-1):
            self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n, n), activation_fn)
        if hlwoa:
            self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(n, output_size*(2**hlwoa)))
            for i in range(hlwoa):
                self.layers = torch.nn.Sequential(self.layers, torch.nn.Linear(output_size*(2**(hlwoa-i)), output_size*(2**(hlwoa-i-1))))
        else:
                ...
                for af in act_fns:
                    for c in lr_update_coeffs:
                        for ui in lr_update_periods:
                            ...
                            if do_analysis:
                                file_name = "Analysis_files/new/isSum-" + repr(s) + "_isAbs-" + repr(a) + "_isPer-" + repr(p) + "_reg-" + repr(R) \
                                            + "_actFun-" + repr(af) + "_lrUpCoeff-" + repr(c) + "_lrUpPer-" + repr(ui) + "_" \
                                ...
                                analysis_file.write(','.join(["Initial Learning Rates"] +  [repr(e/c) for e in init_lrs] + ["\n"]))
                                analysis_file.write(','.join(["Learning Rate update coeff", repr(c), "\n"]))
                                analysis_file.write(','.join(["Learning Rate update period", repr(ui), "\n", "\n"]))
                                analysis_file.write(','.join(["Number of hidden layers with activation"] + [repr(e) for e in num_hl_with_act] + ["\n"]))
                                analysis_file.write(','.join(["Number of units per hidden layer with activation"] + [repr(e) for e in num_units_with_act] + ["\n"]))
                                analysis_file.write(','.join(["Activation Function", repr(af), "\n"]))
                                analysis_file.write(','.join(["Number of hidden layers without activation"] + [repr(e) for e in num_hl_without_act] + ["\n", "\n"]))
                                ...
                                analysis_file.write(','.join(["Initial Learning Rate", "Hidden layers with activation", "Units per hidden layer with activation", "Hidden layers without activation", "e1", "e2", "e3", "e4", "e5", "e6", "\n"]))
                            ...
                            for r in init_lrs:
                                for l1 in num_hl_with_act:
                                    for n in num_units_with_act:
                                        for l2 in num_hl_without_act:
                                            ############################################### Defining model ###############################################
                                                                                     
                                            parameters_arr = np.array([r, l1, n, l2]).reshape((1, min_percent_err_parameters.shape[1]))

                                            model = Model(dim_input, dim_output, l1, n, l2, af)
                                            model.to(device)
                                            ...
                                            fl_nm = "Analysis_files/new/isSum-" + repr(s) + "_isAbs-" + repr(a) + "_isPer-" + repr(p) + "_reg-" + repr(R) \
                                                        + "_ilr-" + repr(r/c) + "_l1-" + repr(l1) + "_n-" + repr(n) + "_l2-" + repr(l2) + "_" \
                                                        + repr(present.year) + "_" + repr(present.month) + "_" + repr(present.day) + "_" + repr(present.hour) + "_" + repr(present.minute)
