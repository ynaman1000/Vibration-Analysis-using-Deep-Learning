...
train_data_frac = 0.001      # fraction of total data points to be used for training
...
val_data_frac = 0.0001       # fraction of total data points to be used for validation
num_iter = 20000

init_lrs = [1e-1, 1e-2, 1e-3]       # initial learning rates 
num_hl_with_act = [2, 4, 8]   # number of hidden layers with activation function
...
num_units_with_act = [32, 64, 128, 256, 512]                  # number of units per hidden layer with activation function
...
lr_update_coeffs = [1, 2, 4, 8]          # number by which learning rate will be divided on every updation
...
save_best_models = False     # for saving best models
                                ...
                                for l1 in num_hl_with_act:
                                    for n1 in num_units_with_act:
                                        for l2 in num_hl_without_act:
                                            if l2 == 0:
                                                num_units_without_act_param = [0]
                                                pad_hl_without_act_front = [False]
                                            for n2 in num_units_without_act_param:
                                                for pad_f in pad_hl_without_act_front:
                                                    ############################################### Defining model ###############################################
                                                                                             
                                                    parameters_arr = np.array([r, l1, n1, l2, n2, pad_f]).reshape((1, min_percent_err_parameters.shape[1]))

                                                    model = Model(dim_input, dim_output, l1, n1, l2, n2, pad_f, af)
                                                    model.to(device)
                                                    ...
                                                    fl_nm = "Analysis_files/new/isSum-" + repr(s) + "_isAbs-" + repr(a) + "_isPer-" + repr(p) + "_reg-" + repr(R) \
                                                                + "_ilr-" + repr(r) + "_l1-" + repr(l1) + "_n1-" + repr(n1) + "_l2-" + repr(l2) +"_n2-" + repr(n2) + "_padF-" + repr(pad_f) + "_" \
                                                                + repr(present.year) + "_" + repr(present.month) + "_" + repr(present.day) + "_" + repr(present.hour) + "_" + repr(present.minute)
