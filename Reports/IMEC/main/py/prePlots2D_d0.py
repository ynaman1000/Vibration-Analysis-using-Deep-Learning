...
datafile_folder = "csv/"
datafile_name = "plot_data_d0-1_2_3_4_5_act"
datafile_ext = ".csv"
...
plt_folder = "plots/"
plt_name = "plot_d0-1_2_3_4_5_act"
...
def plotter(x_data, y_data, fig_nm, subplot_rows, subplot_cols, y_max):
    ...
    for i in range(subplot_rows):
        for j in range(subplot_cols):
            for k in range(numE):
                l[k], = ax[i][j].plot(x_data.reshape((numN)), y_data[k, :, cnt].reshape((numN)), ls='-', lw=0.5, color=clrs[k])
...
plotter(x_data, y_data, plt_folder+plt_name, num_subplot_rows, num_subplot_cols, 1)
