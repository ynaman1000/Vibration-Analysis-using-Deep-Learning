...
numDataPts = 15000*1001     # total number of data points
case = 2
assert case in [1, 2, 3], case
if case == 1:
    miniBatchSize = (2**11)     # size of mini-batch used for training
    numBatch = 2              # number of mini-batches in which training data will be divided for training
    valid_size = 1000               # size of validation dataset
elif case == 2:
    miniBatchSize = (2**11)     # size of mini-batch used for training
    ...
    numBatch = 228              # number of mini-batches in which training data will be divided for training
...
numIter = 9000            # number of iterations for training
...
if numHypParamSamples == 1:
    ...
    lrUpdatePeriod = 500       # number of iterations after which learning rate will be updated
    L1 = 7                   # number of hidden layers with activation function
    N1 = 207                 # number of units per hidden layer with activation function
    ...
    log10initLRInt = [-3, -2]
    lrUpdateCoeffInt = [2, 4]
    lrUpdatePeriodInt = [6000, 7000]
    L1Int = [7, 10]
    N1Int = [200, 250]
    L2Int = [1, 2]
    N2Int = [3, 4]
...
y_ax_lim_loss = 50        # max value on y-axis for loss plots
...
def plotter(err_arr, fig_nm, subplot_rows, subplot_cols, y_max, addTitle=None):
    ...
    if subplot_cols == 1 or subplot_rows == 1:
        if subplot_cols == 1 and subplot_rows == 1:
            ...
            for a in ax:
                a.plot(np.linspace(0, err_arr.shape[1]-1, err_arr.shape[1]).reshape((err_arr.shape[1]))*fps, err_arr[cnt, :].reshape((err_arr.shape[1])), lw=0.3)
...
for opt in optimizers: 
    for aF in actFuns:
        ...
        if do_analysis:
            file_name = "Analysis_files/final_tuning/Case" + repr(case) + "_L1_N1/" + "LN-" + repr(doLayerNorm) + "_opt-" + repr(opt) + "_actFun-" + repr(aF) + "_case-" + repr(case) \
        ...
        for _i in range(numHypParamSamples):
            ...
            fl_nm = "Analysis_files/final_tuning/Case" + repr(case) + "_L1_N1/" + "initLR-" + repr(initLR) + "_lrUC-" + repr(lrUpdateCoeff) + "_lrUP-" + repr(lrUpdatePeriod) \
                    + "_L1-" + repr(L1) + "_N1-" + repr(N1) + "_L2-" + repr(L2) +"_N2-" + repr(N2) + "_" \
                    + repr(present.year) + "_" + repr(present.month) + "_" + repr(present.day) + "_" + repr(present.hour) + "_" + repr(present.minute) + "_" + repr(present.second)

            if (do_anim or plt_max_percent_dev or plt_loss or plt_99per_percent_dev or save_99per_per_dev_data or save_max_per_dev_data):
